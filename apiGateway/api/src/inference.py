...
import api.src.slowfast.utils.checkpoint as cu
...
if len(sys.argv) > 1 and sys.argv[1] == "runserver":
    # If running the server, don't parse additional arguments
    args = type('Args', (object,), {'cfg_file': None, 'opts': None})()  # Create a dummy object
else:
    args = parse_args()

# Set up the configuration file path
base_dir = os.path.dirname(os.path.abspath(__file__))  # Get the directory of the current file
cfg_file_path = os.path.join(base_dir, 'config_32.yaml')
args.cfg_file = cfg_file_path

# Load configuration
cfg = load_config(args)
cfg.UNIFORMER.PRETRAIN_NAME = os.path.join(base_dir, cfg.UNIFORMER.PRETRAIN_NAME)
...
def load_model(model_loaded, label_encoder):
